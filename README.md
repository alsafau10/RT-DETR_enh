...
- `url`<sup>`*`</sup> is the url of pretrained weights convert from paddle model for save energy. _It may have slight differences between this table and paper_
...
python tools/detects.py \
  ...
  --data-root path/to/data/test/ \
  --ann-file path/to/data/test/annotations.coco.json \
  --output-dir path/for/the/output \
  --enhance-method [linear, clahe] \
  --he-threshold 1.1 #untuk probabilitas 100 karena ingin melihat hasil deteksinya
