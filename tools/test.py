...
def main(args):
    # initialize distributed for consistency (even if single GPU)
    ...
    # build solver for postprocessor and device
    ...
    # decide per-run if we apply enhancement
    ...
    record = out_dir / f"eval_{timestamp}.txt"
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--batch-size',   '-bs', type=int, default=4, help='batch size for eval')
    #parser.add_argument('--num-workers',  '-nw', type=int, default=4, help='DataLoader workers')
    #parser.add_argument('--img-size',          type=int, default=640,  help='resize shorter side')
